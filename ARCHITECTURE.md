...
| `/preview/tenants` | `pages/preview/tenants.vue` | **Tenants (CRM)** — landlord-only. A tenant directory derived from `bookings.json` (grouped by company, scoped to the active team) merged with prospects from `/api/tenants`. Per tenant: derived **pipeline stage** (Active / Lead / Prospect / Lapsed via `StatusDot`), lifetime value (landlord net), bookings count, "Repeat" tag, centres used, and a **renewal-due** flag (confirmed booking ending ≤45 days out with no follow-on). Stage tabs + search + a "Renewals due" filter. Row → **tenant-360 overlay** (centered modal) with **Overview / People tabs**: Overview = primary contact, lifetime value / bookings / outstanding / payment reliability, their bookings & enquiries, per-tenant notes, quick actions (Message · View bookings → `/bookings?q=<company>` · Create booking); People = all contacts at the company (role, email, phone, Primary badge, per-contact booking count). Companies can have multiple people — the table shows the primary contact + "+N more"; contacts come from `/api/contacts` (keyed by company) merged with booking contacts (`contactsForCompany`). Company-first by design; a standalone Users page is a clean later add. CRM holds *one tenant's world*; portfolio aggregates belong to the (upcoming) Analytics page. Nav item sits under Transactions (id `crm`, label "Tenants"). |
...
| GET | `/api/contacts` | `server/data/contacts.json` | Per-company contact rosters keyed by lowercased company name (name, role, email, phone, `primary`) — lets a company have multiple people beyond the single booking contact. Merged into each tenant by the Tenants page. Served by `server/api/contacts.get.ts`. |
