...
| GET | `/api/companies` | `server/data/companies.json` | Company master records keyed by lowercased company name: the company's own `category` (sector, e.g. Sportswear / Beauty / Technology) and its `contacts[]` roster (name, role, email, phone, `primary`). The Tenants page enriches bookings-derived companies with this (bookings only carry booking-type data). Served by `server/api/companies.get.ts`. |
