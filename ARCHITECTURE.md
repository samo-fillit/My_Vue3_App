...
| `/preview/tenants` | `pages/preview/tenants.vue` | **Tenants (CRM)** — landlord-only. A tenant directory derived from `bookings.json` (grouped by company, scoped to the active team) merged with prospects from `/api/tenants`. Per tenant: company-level **category** (sector, from `/api/companies` — not derived from booking types), derived **pipeline stage** (Active / Lead / Prospect / Lapsed via `StatusDot`), lifetime value (landlord net), bookings count, "Repeat" tag, and a **renewal-due** flag (confirmed booking ending ≤14 days out with no follow-on; the overlay callout links to Message tenant + View booking). Stage tabs + search + a "Renewals due" filter. Columns: Company · Primary contact (name + role) · Category · Bookings · Lifetime value · Stage · renewal flag (no Centres column). Row → **tenant-360 overlay** (centered modal) with **Overview / People tabs**: Overview = primary contact, a **Transactions** block (financial summary — lifetime value / bookings / outstanding / payment reliability — plus upcoming-payment count, an overdue-payments list, and a View-all link to `/transactions?q=<company>`), their bookings & enquiries, per-tenant notes, quick actions (Message · View bookings → `/bookings?q=<company>` · Create booking); People = all contacts at the company (role, email, phone, Primary badge, per-contact booking count). Companies can have multiple people — the table shows the primary contact + "+N more"; contacts come from `/api/contacts` (keyed by company) merged with booking contacts (`contactsForCompany`). Company-first by design; a standalone Users page is a clean later add. CRM holds *one tenant's world*; portfolio aggregates belong to the (upcoming) Analytics page. Nav item sits under Transactions (id `crm`, label "Tenants"). |
| `/preview/invoices` | `pages/preview/invoices.vue` | Invoices list with date range picker, search bar, and sortable table (tenant) |
| `/preview/booking-links` | `pages/preview/booking-links.vue` | Booking links history with Create booking overlay, centre filter, and search bar; linked from "Create Link" sidebar item. **Company-first columns**: Company (primary, bold) then Contact (name + email, secondary). A trailing **Type** column tags a link as **Renewal** (`isRenewal`) — auto-set for links created via the bookings Renew flow, and manually toggleable (`toggleRenewal`). The **Create booking overlay is company-first** (a booking belongs to the tenant company; any company email can be used): a Company autocomplete over the company master (`/api/companies`) — selecting one surfaces its contacts as quick-pick chips and lets you enter any new email under that company; new companies are allowed too. Arriving with `?create=1` (e.g. the bookings page CTA, or the bookings Renew flow) auto-opens the create overlay after a 500ms delay — prefilled from `useState('renewal-draft')` when it's a renewal hand-off. Statuses: Sent / Declined / **Enquiry created** (the last = tenant completed the link, creating an enquiry/booking; underlying status value remains `completed`) |
...
| `/preview/bookings` | `pages/preview/bookings/index.vue` | Bookings list — role-aware (landlord scopes by active team + sees tenant identity + "Create booking" CTA; tenant sees own bookings, second-person status labels, no create). Tabs: Action needed / Upcoming / Past / Closed (mutually exclusive buckets derived from the taxonomy). Split Start/End date columns (independently sortable), centre + date-range + search filters, centre-colour avatars. Status shown as a **coloured dot + label** (colour lives in the dot, label stays foreground; neutral states use `bg-muted-foreground`), with a pulsing dot on active "Live now" bookings. Bookings with an overdue payment (`financials.paymentStatus === 'overdue'`) are pulled into Action needed regardless of temporal state, keeping their lifecycle pill plus a red flag icon (tooltip: "Payment overdue") shown in a dedicated **notifications column** between Status and the notes column. That column also carries a clash-warning triangle and, for renewable bookings (`isRenewable`: confirmed, ending ≤14 days, no later confirmed booking for the same tenant), an amber **Renew** button → a renew overlay that proposes a new term (date picker blocking dates booked on that space) + rate and, on confirm (`confirmRenew`), hands the prefilled details to the booking-links create flow via `useState('renewal-draft')` and navigates to `/booking-links?create=1` — i.e. the renewal goes out as a booking-link enquiry (becomes a booking once the tenant completes it), not a direct booking. Primary CTA "Create booking" (landlord). Clicking a row opens the **booking detail overlay** (centered modal — never a right slide-over, which would clash with the notifications/assistant right panel): header (centre/space + status + ID), key facts, financials (role-aware: tenant "Total" vs landlord "You receive" after the Fillit fee), payment schedule, documents, enquiry, manager-approval chain (Nhood), activity timeline, notes — and a status- and role-aware CTA footer (see below). The overlay also has: a **transactions summary** (collected/outstanding) + "View transactions" button that deep-links to `/transactions?q=<bookingId>` (pre-fills its search); inline **rate negotiation** on enquiries (landlord adjusts the rate — VAT/fee/totals re-derive live via `deriveFinancials` — then "Send to tenant" → `quoted`; tenant accepts → `confirmed` or declines → `declined`); and an inline **payment schedule** shown on every booking (`displayPayments` synthesises a proposed single "Full payment" from the gross total when none is stored, e.g. a new enquiry; amounts render in a CSS grid so they align vertically). The schedule is **editable only before the booking starts** (`scheduleEditable`: landlord, not closed, future `period.from`) — landlord; paid instalments locked (only upcoming payments change), add/remove rows, amounts must sum to the booking total and dates be ordered/in-window, Save gated by `scheduleValid`. **Notes** are per-viewer (landlord/tenant each have their own): a note icon in the final table column fills yellow when the viewer has a note, with a popover editor; the viewer's note also shows in the overlay. Reads `/api/bookings`. |
...
| GET | `/api/bookings` | `server/data/bookings.json` | 28 booking records on the redesigned booking-status taxonomy (below). Production-style nested shape: `landlord`, `tenant`, `space`(+centre), `enquiry`(pitch), `financials`, `payments[]`, `documents[]`, `docusign`, `managerApproval` (Nhood), `actions[]` audit trail, `notes`. Served by `server/api/bookings.get.ts` — valid JSON (no `//` comments) so it `JSON.parse`s directly. |
| GET | `/api/tenants` | `server/data/tenants.json` | CRM tenant records — primarily **prospects** (leads with no bookings yet) plus CRM-only fields (`source`, `note`, `category`, `teamId`). The Tenants page merges these with companies derived from `/api/bookings` for live numbers. Served by `server/api/tenants.get.ts`. |
| GET | `/api/companies` | `server/data/companies.json` | Company master records keyed by lowercased company name: proper-case `name`, the company's own `category` (sector, e.g. Sportswear / Beauty / Technology) and its `contacts[]` roster (name, role, email, phone, `primary`). Used by the Tenants page (enriches bookings-derived companies) and the booking-links company-first create flow. Served by `server/api/companies.get.ts`. |
...
**Nhood lease-signature flow** (eLeaseLoop only; `leaseSent` / `docusignMeta` / `sendForSignature` / `showLeaseSection`): mirrors production's DocuSign-gated confirmation. Fillit confirms directly (Accept quote → confirmed); on eLeaseLoop, Accept quote → `awaiting_signature` (lease not yet sent), landlord **Send for signature** sets `docusign.status='sent'`, tenant **Sign lease** (disabled until sent) sets it `completed` → `confirmed`. The "Lease signature" overlay section shows the envelope status + dates + id. `isActionNeeded` routes an unsent accepted lease to the landlord's Action-needed (they own the send step) and treats the tenant as waiting. Cancelling a live envelope voids it; the Wave 1 `cancelLocked` blocks cancel once signed. **External Ref ID** (`externalRef`, Yardi/ERP reference) is an inline editable field shown only for Nhood landlords.
